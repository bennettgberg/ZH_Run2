...
def getArgs() :
    ...
    parser.add_argument("-s","--selection",default='HAA',help="is this for the ZH or the AZH analysis?")
    ...
    for isyst, systematic in enumerate(sysT) :
        ...
        for lepMode in ['mm'] :
            ...
            if lepMode == 'ee' :
                ...
                pairList, lepList = TF.findLeadMuMu(goodElectronList, e)
                for cat in cats[:4] :
            ...
            if lepMode == 'mm' :
                ...
                pairList, lepList = TF.findLeadMuMu(goodMuonList, e)
                for cat in cats[4:] :
                    cutCounter[cat].count('LeptonPair')
                    if  MC :   cutCounterGenWeight[cat].countGenWeight('LeptonPair', e.genWeight)
            ...
            for tauMode in ['et','mt','tt','em'] :
                ...
                outTuple.Fill(e,SVFit,cat,jt1,jt2,LepP,LepM,lepList,fullLepList,isMC,era,doJME, met_pt, met_phi,  isyst, tauMass, tauPt, eleMass, elePt, muMass, muPt)
