...
for count, e in enumerate(inTree) :
    ...
    for cat in cats:
        #if the cat name is tttt, this is 4tau event. Find the pair with the highest sum pT.
        if cat == 'tttt' :
            tauList = tauFun_4tau.getTauList(cat, e, pairList=pairList, printOn=True)
            print("before calling get2BestTauPairPt, tauList={}".format(str(tauList)))
            #make a function to get 2 best tau pairs instead of just 1!
            bestTauPair = tauFun_4tau.get2BestTauPairPt(cat, e, tauList)
            #print out these 2 tau pairs.
            if bestTauPair != []: print("\n\n2 Tau pairs: {}".format(str(bestTauPair)))
        elif cat[2:] == 'tt' :
        ...
        if len(bestTauPair) > 1 :
            ...
            if bestTauPair and args.genMatch:
                ...
                if cat == 'tttt':
                    ...
                    motherIndex3  = tauFun.findAMother(e,36,tauIdx3)
                    motherIndex4  = tauFun.findAMother(e,36,tauIdx4)
